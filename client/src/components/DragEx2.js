...
const Board = () => {
  ...
  ]); // Provide your initial job data here
...
//   ];
  ...
  const getStatusFromList = (job, list) => {
    if (list === status1Jobs) {
      return 1;
    } else if (list === status2Jobs) {
      return 2;
    } else if (list === status3Jobs) {
      return 3;
    }
    return job.status;
  };
  ...
  const onDragEnd = (result) => {
    ...
    const reorderedItem = sourceList.splice(result.source.index, 1)[0];
    destinationList.splice(result.destination.index, 0, reorderedItem);
  
    const newJobData = jobData.map((job) => ({
      ...job,
      status: getStatusFromList(job, destinationList),
    }));
  
    setJobData(newJobData); // Update the state with the new job data
  };
        ...
        <Row className="d-flex justify-content-around">
          <Col md={4}>
            <Card className="column-status my-3">
              ...
              <Droppable droppableId="status1" key="status1">
                {(provided) => (
                  <Card.Body
          ...
          <Col md={4}>
            <Card className="column-status my-3">
              ...
              <Droppable droppableId="status2" key="status2">
                {(provided) => (
                  <Card.Body
                    {...provided.droppableProps}
                    ref={provided.innerRef}
          ...
          <Col md={4}>
            <Card className="column-status my-3">
              ...
              <Droppable droppableId="status3" key="status3">
                {(provided) => (
                  <Card.Body
                    {...provided.droppableProps}
                    ref={provided.innerRef}
